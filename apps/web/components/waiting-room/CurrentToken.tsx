  ...
  return (
    <section className="overflow-hidden rounded-lg border border-white/10 bg-surface/70 p-6 shadow-glow backdrop-blur-xl sm:p-8">
      ...
      <motion.div
        ...
        className="mt-8 flex min-h-44 flex-col items-center justify-center rounded-lg border border-primary/20 bg-primary/10 p-6 text-center"
      >
        ...
        <div className="mt-4 space-y-1">
          <p className="text-xl font-semibold">{patient?.patientName ?? "No patient is being served"}</p>
          <p className="text-sm text-muted-foreground">
            {patient ? `${patient.doctorName} - Room ${patient.room}` : "Waiting for the next token"}
          </p>
        </div>
      </motion.div>
    </section>
